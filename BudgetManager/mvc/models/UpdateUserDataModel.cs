...
namespace BudgetManager {
    class UpdateUserDataModel : IUpdaterModel {
        ...
        String sqlStatementSelectFullYearSavingAccountExpenses = @"SELECT expenseID AS 'ID', name AS 'Name', (SELECT categoryName FROM expense_types WHERE categoryID = type) AS 'Expense type', value AS 'Value', date AS 'Date' FROM `saving_accounts_expenses` WHERE account_ID = (SELECT accountID FROM accounts WHERE user_ID = @paramID AND accountName LIKE '%SYSTEM_DEFINED%') AND YEAR(date) = @paramYear ORDER BY date ASC";
